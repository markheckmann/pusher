...
test_that("overview_summary prints the management summary and tracked repo count", {
  with_pusher_home({
    fixture <- make_repo()
    commit_file(fixture$repo, "initial.txt", "initial", "2020-01-01T00:00:00+0000")
    git(fixture$repo, c("push", "-u", "origin", "main"))
    ...
    expect_match(output[[5]], "Tracked repos/branches: 1", fixed = TRUE)
    expect_false(any(grepl(normalizePath(fixture$repo), output, fixed = TRUE)))
    expect_false(any(grepl("main -> origin/main", output, fixed = TRUE)))
...
test_that("overview_summary reports no tracked repos", {
  with_pusher_home({
    ...
    expect_match(output[[5]], "Tracked repos/branches: 0", fixed = TRUE)
  })
})
